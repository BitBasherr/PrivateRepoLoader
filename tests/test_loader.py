...
@pytest.fixture def tmp_repo(tmp_path: Path) -> Path: """Create and commit in a temporary Git repo on branch 'main'.""" repo_dir = tmp_path / "repo" repo = git.Repo.init(repo_dir) (repo_dir / "README.md").write_text("# test") repo.index.add(["README.md"]) repo.index.commit("initial") repo.git.branch("-M", "main") return repo_dir
...
def test_sync_update(tmp_repo: Path, tmp_path: Path) -> None: """sync_repo should pull new commits onto an existing clone.""" cfg = { "repository": tmp_repo.as_uri(), "slug": "r", "branch": "main", "token": "", } dest_root = tmp_path / "out" dest_root.mkdir() first = sync_repo(dest_root, cfg) assert first == "cloned"
